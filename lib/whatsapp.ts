...
export function mensajeSolicitudCita(nombre: string, fechaTexto: string, horaTexto: string) {
  const primerNombre = nombre.split(" ")[0];
  return `¡Hola, ${primerNombre}! 🦷 Recibimos tu solicitud de cita para el ${fechaTexto} a las ${horaTexto} en Viña Sonrisas. En breve te confirmamos por este medio.`;
}
