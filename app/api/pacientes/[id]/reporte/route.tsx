...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { rows: pacienteRows } = await query<{
      ...
      `SELECT id, nombre, telefono, email, folio, fecha_nacimiento, creado_en, alergias, medicamentos, antecedentes_medicos
    ...
    const { rows: historiaRows } = await query(
      `SELECT ${HISTORIA_CLINICA_COLUMNAS} FROM historia_clinica WHERE paciente_id = $1`,
      [pacienteId]
    );
            ...
            {!historia && !paciente.alergias && !paciente.medicamentos && !paciente.antecedentes_medicos ? (
              ...
              <View>
                ...
                {(paciente.medicamentos || historia?.toma_medicamento) && (
                  <Text style={estilosPdf.parrafo}>
                    • Medicamentos actuales: {paciente.medicamentos || historia?.toma_medicamento || "—"}
                  </Text>
                )}
