...
// "Eliminar" no borra (NOM-024: nada se borra de verdad) — marca el
// presupuesto vigente=false con el motivo y quién lo anuló. Sigue
// existiendo en la base para auditoría.
  ...
  try {
    ...
    const { rows } = await query<{ id: number }>(
      `UPDATE presupuestos
       ...
       RETURNING id`,
      [motivo, identidad.nombre, presupuestoId, pacienteId]
    );
    if (rows.length === 0) {
